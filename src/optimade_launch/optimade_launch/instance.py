...
class OptimadeInstance:
    ...
    def create(self, data: bool = False) -> Container:
        ...
        if self.profile.unix_sock is not None:
            ...
            params_container["volumes"] = {
                host_sock_folder: {"bind": "/tmp", "mode": "rw"}
            }
    ...
    async def _web_service_online(self, container: Container) -> str:
        ...
        def _get_curl_cmd(assumed_protocol, port=None, unix_sock=None):
            cmd = "curl --fail-early --fail --silent --max-time 1.0 "
            if port is not None:
                cmd += f"{assumed_protocol}://localhost:{port}/v1/info"
            elif unix_sock is not None:
                cmd += (
                    f"{assumed_protocol}://localhost/v1/info --unix-socket {unix_sock}"
                )
            return cmd

        while True:
            try:
                curl_cmd = _get_curl_cmd(
                    assumed_protocol,
                    port=self.profile.port,
                    unix_sock=self.profile.unix_sock,
                )
                LOGGER.debug(f"Checking if online: '{curl_cmd}'")
